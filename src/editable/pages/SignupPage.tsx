  ...
  return (
    <EditableSiteShell>
      <main className="bg-[var(--slot4-panel-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto grid min-h-[calc(100vh-12rem)] max-w-[var(--editable-container)] items-center gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[0.95fr_1fr] lg:px-8">
          <div className="rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_4px_24px_rgba(0,0,0,0.08)] sm:p-9">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slot4-accent)] lg:hidden">{pagesContent.auth.signup.badge}</p>
            <h1 className="mt-2 text-2xl font-bold tracking-[-0.01em] lg:mt-0">{pagesContent.auth.signup.formTitle}</h1>
          ...
          <div className="hidden rounded-3xl border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-10 text-[var(--slot4-page-text)] lg:block lg:min-h-[520px] lg:p-14">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--slot4-muted-text)]">{pagesContent.auth.signup.badge}</p>
            <h2 className="editable-display mt-5 max-w-md text-4xl font-semibold leading-[1.06] tracking-[-0.03em] sm:text-5xl">{pagesContent.auth.signup.title}</h2>
            <p className="mt-5 max-w-sm text-base leading-7 text-[var(--slot4-muted-text)]">{pagesContent.auth.signup.description}</p>
